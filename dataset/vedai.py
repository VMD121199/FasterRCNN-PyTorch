import glob
import os
import random

import torch
import torchvision
from PIL import Image
from tqdm import tqdm
from torch.utils.data.dataset import Dataset
import xml.etree.ElementTree as ET

def yolo_to_bbox(box, img_width, img_height):
    """
    Converts YOLO format bounding boxes to (x1, y1, x2, y2) format.

    Parameters:
        yolo_boxes (list of lists): List of bounding boxes in YOLO format 
                                    (x_center, y_center, width, height).
        img_width (int): Width of the image.
        img_height (int): Height of the image.

    Returns:
        list of tuples: List of bounding boxes in (x1, y1, x2, y2) format.
    """
    x_center, y_center, width, height = box[0], box[1], box[2], box[3]
    x1 = (x_center - width / 2) * img_width
    y1 = (y_center - height / 2) * img_height
    x2 = (x_center + width / 2) * img_width
    y2 = (y_center + height / 2) * img_height
    new_box = [int(x1), int(y1), int(x2), int(y2)]
    return new_box


def load_images_and_anns(im_dir, ann_dir, label2idx):
    r"""
    Method to get the xml files and for each file
    get all the objects and their ground truth detection
    information for the dataset
    :param im_dir: Path of the images
    :param ann_dir: Path of annotation xmlfiles
    :param label2idx: Class Name to index mapping for dataset
    :return:
    """
    im_infos = []
    for ann_file in tqdm(glob.glob(os.path.join(ann_dir, '*.txt'))):
        im_info = {}
        im_info['img_id'] = os.path.basename(ann_file).split('.txt')[0]
        im_info['filename'] = os.path.join(im_dir, '{}_co.png'.format(im_info['img_id']))
        im_info['filename_ir'] = os.path.join(im_dir, '{}_ir.png'.format(im_info['img_id']))
        with open(ann_file, 'r')as f:
            ann_info = f.readlines()
        width = 512
        height = 512
        im_info['width'] = width
        im_info['height'] = height
        detections = []
        for obj in ann_info:
            obj = obj.split(' ')
            det = {}
            bbox = [float(obj[1]), float(obj[2]), float(obj[3]), float(obj[4])]
            bbox = yolo_to_bbox(bbox, width, height)
            det['label'] = int(obj[0])
            det['bbox'] = bbox
            detections.append(det)
        im_info['detections'] = detections
        im_infos.append(im_info)
    print('Total {} images found'.format(len(im_infos)))
    return im_infos


class VEDAIDataset(Dataset):
    def __init__(self, split, im_dir, ann_dir):
        self.split = split
        self.im_dir = im_dir
        self.ann_dir = ann_dir
        classes = ["car", "pickup", "camping cars", "trucks", "others", "tractors", "boats", "vans"]
        self.label2idx = {classes[idx]: idx for idx in range(len(classes))}
        self.idx2label = {idx: classes[idx] for idx in range(len(classes))}
        print(self.idx2label)
        self.images_info = load_images_and_anns(im_dir, ann_dir, self.label2idx)
    
    def __len__(self):
        return len(self.images_info)
    
    def __getitem__(self, index):
        im_info = self.images_info[index]
        im = Image.open(im_info['filename'])
        to_flip = False
        if self.split == 'train' and random.random() < 0.5:
            to_flip = True
            im = im.transpose(Image.FLIP_LEFT_RIGHT)
        im_tensor = torchvision.transforms.ToTensor()(im)
        targets = {}
        targets['bboxes'] = torch.as_tensor([detection['bbox'] for detection in im_info['detections']])
        targets['labels'] = torch.as_tensor([detection['label'] for detection in im_info['detections']])
        if to_flip:
            for idx, box in enumerate(targets['bboxes']):
                x1, y1, x2, y2 = box
                w = x2-x1
                im_w = im_tensor.shape[-1]
                x1 = im_w - x1 - w
                x2 = x1 + w
                targets['bboxes'][idx] = torch.as_tensor([x1, y1, x2, y2])
        return im_tensor, targets, im_info['filename']
        